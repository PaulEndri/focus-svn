...
export class JiraService {
  ...
  constructor(config) {
		this.auth = Buffer.from(`${config.jiraUsername}:${config.jiraToken}`).toString('base64');
	...
	async updateBranch(project, ticketNumber, branchName) {
      ...
      throw new Error(`Failed to update branch name for ${project}-${ticketNumber}: ${error.message}`);
    }
	}
  /**
    ...
    try {
      ...
      return {
        ...
        priority: issue.fields.priority?.name,
				...
				revision: issue.fields?.customfield_13541?.match(this.versionRegex)?.[1]
      };
    } catch (error) {
			console.log(error);
			throw new Error(`Failed to get ticket info for ${this.projectId}-${ticketNumber}: ${error.message}`);
    }
  }
}
